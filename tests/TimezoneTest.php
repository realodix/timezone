...
class TimezoneTest extends TestCase
{
    ...
    #[PHPUnit\Test]
    public function testToSelectBox_HasGeneralGroup(): void
    {
        $expected = '<optgroup label="General"><option value="UTC">(UTC+00:00)&nbsp;&nbsp;&nbsp;UTC</option></optgroup>';

        $output = $this->tz->toSelectBox('');
        $this->assertStringContainsString($expected, $output);

        $output = $this->tz->onlyGroups(['General'])->toSelectBox('');
        $this->assertStringContainsString($expected, $output);

        $output = $this->tz->excludeGroups(['General'])->toSelectBox('');
    ...
    public function testIsGrouped(): void
    {
        ...
        $this->assertStringContainsString('<optgroup', $outputWithGroup, 'Asserting optgroup tag exists when isGrouped is true');

        $outputWithoutGroup = $this->tz->flatten()->toSelectBox('timezone_split_false');
        $this->assertStringNotContainsString('<optgroup', $outputWithoutGroup, 'Asserting optgroup tag does not exist when isGrouped is false');
    }
