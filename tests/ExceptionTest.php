...
class ExceptionTest extends TestCase
{
    ...
    /**
     * This test should throw an exception when a timezone UTC is selected
     * that is not within the filter scope.
     */
    public function testThrowsExceptionForUTCTimezoneOutsideFilterScope(): void
    {
        $tz = $this->tz->onlyGroups(['Asia', 'America']);
        $this->expectException(TimezoneOutOfScopeException::class);
        $this->expectExceptionMessage(
            'Timezone "UTC" is not within the specified groups: America, Asia',
        );
        $tz->toSelectBox('timezone_default', 'UTC');
    }
}
